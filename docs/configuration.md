...
`cadence-config` 是无机器人依赖的小包，PlanetJ、PlanetR、Planet-PingPong 与应用入口共同使用。配置加载不扫描相邻仓库。
...
`cadence-rally` 兼容旧配置中明确的 `configs/`、`assets/`、`models/`、`contracts/` 前缀：它们相对于应用 bundle。新写的其他相对路径按声明来源解析。Planet-PingPong 的旧配置也保留显式 bundle 引用，默认资源已随 wheel 分发。修改新应用的 bundle 通过 `CADENCE_RALLY_BUNDLE` 显式选择，不依赖运行目录。
