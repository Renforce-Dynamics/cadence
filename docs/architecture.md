...
| `cadence` | Runtime, safety, state lifecycle, command composition, generic motion, inference, simulation backends, configuration and operator protocol | Optional A3 SDK |
...
| `cadence-rally` | Rally execution states, behavior orchestration, task models, robot/site deployments and process composition | Cadence and planet-rally |
...
| `cadence-protocol` | Canonical PLNJ codec and small operator/upper-target UDP clients; standard library only |
...
Only the command accepted by the backend advances committed state progress.
