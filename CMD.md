...
| --- | --- | --- |
| `examples/entry_mock.yaml` | 双关节内存后端 | 无 |
| `examples/entry_sim.yaml` | 双关节 MuJoCo 仿真 | `sim` |
| `a3/mock/entry_a3_lower.yaml` | A3 下肢策略与固定上肢，内存后端 | `inference` |
| `a3/mock/entry_a3_lower_stream.yaml` | A3 下肢策略与实时上肢，内存后端 | `inference` |
| `a3/mock/entry_a3_operator.yaml` | 手柄请求通用 A3 状态，固定上肢 | `inference` |
| `a3/mock/entry_a3_operator_stream.yaml` | 手柄请求通用 A3 状态，实时上肢 | `inference` |
| `a3/mock/entry_sdk_mock.yaml` | native SDK 静止状态与内存写确认 | `a3`、`inference` |
| `a3/onboard/entry_onboard_a3_real_readonly.yaml` | A3 真实状态，只读计算 | `a3`、`inference`、AimRT |
| `a3/onboard/entry_onboard_a3_real.yaml` | A3 真实状态与命令 | 同上 |
| `a3/onboard/entry_onboard_upper_stream_readonly.yaml` | 真实状态与实时上肢，只读计算 | 同上 |
| `a3/onboard/entry_onboard_upper_stream.yaml` | A3 命令模式，实时上肢 | 同上 |
...
实时上肢 runtime 可换用 `a3/mock/entry_a3_operator_stream.yaml`，手柄入口相同。
`joystick/entry_joystick.yaml` 只引用本仓库 `configs/operators/joystick.yaml`，由 PlanetJoystick 读取；
...
需要现场差异时，修改所选配置；需要保留独立版本时，在 `configs/entry/a3/onboard/entry_site.yaml`
...
.venv/bin/cadence config diff configs/entry/examples/entry_mock.yaml configs/entry/examples/entry_sim.yaml
./scripts/doctor.sh
./scripts/test.sh
./scripts/build.sh
